...
var validator = (function ($) {
	...
	*/
    function checkField() {
        ...
        ////Change by Akbar Before
        //if( this.type !='hidden' && $(this).is(':hidden') )
        //	return true;
        // after new code 26 /12/2016
        if (this.type != 'hidden' && $(this).is(':hidden:not(.hidden-validation)'))
            return true;
        ///Change By Akbar
	...
	*/
    ...
    /// New added by Akbar, For  chosen Select Item Hidden  Validation 
    function checkAllWithHide($form) {
        $form = $($form);
        ...
        var that = this,
			submit = true, // save the scope
            fieldsToCheck = $form.find(':input').setDefaults({ ignore: ":hidden:not(.hide-validation)" });
        fieldsToCheck = fieldsToCheck.find(':input').filter('[required=required], .required, .optional').not('[disabled=disabled]');
    ...
    return {
        ...
        keypress: keypress,
        checkAll: checkAll,
        checkAllWithHide: checkAllWithHide,
        mark: mark,
        unmark: unmark,
        message: message,
        tests: tests
