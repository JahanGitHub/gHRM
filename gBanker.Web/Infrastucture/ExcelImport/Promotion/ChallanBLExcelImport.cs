...
{
    public class ChallanBLExcelImport : IExcelImport
    {
            ...
            foreach (var sheet in Worksheets)
            {
                ...
                for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                {
                    ...
                    try
                    {
                        ...
                        string Year = ExcelImportHelper.GetStringFromExcelCell(sheet, rowIterator, 2, "Year", out Message);
                        string Month = ExcelImportHelper.GetStringFromExcelCell(sheet, rowIterator, 3, "Month", out Message);
                        string Date = ExcelImportHelper.GetStringFromExcelCell(sheet, rowIterator, 4, "Date", out Message);

                        string ChallanNo = ExcelImportHelper.GetStringFromExcelCell(sheet, rowIterator, 5, "Challan No", out Message);

                        string ChallanAmount = ExcelImportHelper.GetStringFromExcelCell(sheet, rowIterator, 6, "Challan Amount", out Message);
                        ...
                        var tt = spService.GetDataWithParameter(param, "prl.sp_insert_update_challan_from_excel");
