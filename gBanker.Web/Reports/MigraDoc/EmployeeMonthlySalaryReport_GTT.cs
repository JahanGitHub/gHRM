...
{
    public class EmployeeMonthlySalaryReport_GTT
    {
        ...
        public EmployeeMonthlySalaryReport_GTT(HttpContextBase Context)
        {
        ...
        public Document GetPdfDocument(string FileName, DataSet DSet)
        {
            ...
            foreach (DataRow DataRow in DSet.Tables[0].Rows)
            {
                ...
                {
                    SerialNo = Convert.ToInt32(DataRow["Serial"]),
                    ...
                    DesignationName = DataRow["DesignationName"].ToString(),
                    DesignationShortName = DataRow["DesignationShortName"].ToString(),
            ...
            Data = Data.OrderBy(x => x.SerialNo).ToList();
        ...
        public Document GetXLDocument(string FileName, DataSet DSet)
        {
            Data = new List<EmployeeMonthlySalaryReport_Data>();
            foreach (DataRow DataRow in DSet.Tables[0].Rows)
            {
        ...
        private void PageSetup(Document Doc, Section Sec)
        {
            ...
            Sec.PageSetup.LeftMargin = new Unit(0.3, UnitType.Inch);
            Sec.PageSetup.RightMargin = new Unit(0.3, UnitType.Inch);
            ...
            Sec.PageSetup.HeaderDistance = new Unit(0.37, UnitType.Inch);
        ...
        private void AddHeader(Section Sec)
        {
            ...
            CName.Format.Font.Size = 11;//Settings.HeaderFontSize;
            CName.AddText(SessionHelper.CompanyName);

            Sec.Headers.Primary.AddParagraph().Format.Font.Size = 2;

            Paragraph CAddress = Sec.Headers.Primary.AddParagraph();
            CAddress.Format.Alignment = ParagraphAlignment.Center;
            CAddress.Format.Font.Size = 9;//Settings.HeaderFontSize;
            CAddress.AddText(SessionHelper.CompanyAddress);

            Sec.Headers.Primary.AddParagraph().Format.Font.Size = 4;
            ...
            Sec.Headers.Primary.AddParagraph().Format.Font.Size = 6;

            Table Tbl = Sec.Headers.Primary.AddTable();
            Tbl.Borders.Width = 0.10;
            Tbl.Format.Font.Size = 6;// Settings.BodyFontSize;
            Tbl.LeftPadding = 0.1;
            Tbl.RightPadding = 0.1;
            AddTableHeader(Tbl);
        }
        ...
        private void AddTableCols(Table Tbl)
        {
            ...
            //TblTotalColCount = 6 + ComponentList.Count() + TransactionTypeList.Count();
            TblTotalColCount = 7 + ComponentList.Count() + TransactionTypeList.Count();
            var totallenght = (SecWidth() - ((TblTotalColCount - 1) * EachColWidth)) / 7;

            for (int i = 0; i < TblTotalColCount; i++)
            {
                double ColWidth = EachColWidth - .03;
                //if (0 == i)
                //  //  ColWidth = totallenght - .00009;
                //if (1 == i)
                 //   ColWidth = totallenght -.00001;
                //if (2 == i)
                //  //  ColWidth = totallenght * 5.75;
                //else if(3 == i)
                //  ColWidth = totallenght * 5.75;
        ...
        private void AddTableHeader(Table Tbl)
        {
            ...
            Cell SerialCell = Row1.Cells[CurrentIndex];
            SerialCell.MergeDown = 1;
            SerialCell.AddParagraph().AddText("Sl.");
            CurrentIndex++;
            ...
            NameDesCell.AddParagraph().AddText("Name");
            CurrentIndex++;

            Cell DesignationCell = Row1.Cells[CurrentIndex];
            DesignationCell.MergeDown = 1;
            DesignationCell.AddParagraph().AddText("Designation");
            CurrentIndex++;

            Cell GrossCell = Row1.Cells[CurrentIndex];
            GrossCell.MergeDown = 1;
            GrossCell.AddParagraph().AddText("Gross Salary");
            CurrentIndex++;
        ...
        private void AddTableBody(Table Tbl)
        {
            ...
            List<string> DepartmentList = Data.Select(x => x.DepartmentName).Distinct().ToList();
            foreach (string OfficeDes in OfficeList)
            {
                ...
                {
                    var DeptGHeaderRow = Tbl.AddRow();
                    Cell DeptGHeaderCell = DeptGHeaderRow.Cells[0];
                    DeptGHeaderCell.Format.Font.Bold = true;
                    DeptGHeaderCell.AddParagraph().AddText(Dept);
                    DeptGHeaderCell.MergeRight = TblTotalColCount - 1;


                    var EmployeeList = Data.Where(x => x.OfficeShortOrder == OfficeDes && x.DepartmentName == Dept).Select(x => new
                    {
                        x.SerialNo,
                        x.EmployeeCode,
                        x.EmployeeName,
                    ...
                    foreach (var Employee in EmployeeList)
                    {
                        ...
                        var Row = Tbl.AddRow();
                        //(Row.Cells[CurrentIndex]).AddParagraph().AddText(Employee.SerialNo.ToString());
                        Cell sl = Row.Cells[CurrentIndex];
                        sl.Format.Alignment = ParagraphAlignment.Center;
                        sl.AddParagraph().AddText(Employee.SerialNo.ToString());
                        CurrentIndex++;
                        //(Row.Cells[CurrentIndex]).AddParagraph().AddText(Employee.EmployeeCode);
                        Cell empCode = Row.Cells[CurrentIndex];
                        empCode.Format.Alignment = ParagraphAlignment.Center;
                        empCode.AddParagraph().AddText(Employee.EmployeeCode.ToString());
                        CurrentIndex++;
                        (Row.Cells[CurrentIndex]).AddParagraph().AddText(Employee.EmployeeName);
                        CurrentIndex++;
                        (Row.Cells[CurrentIndex]).AddParagraph().AddText(Employee.DesignationName);
                        CurrentIndex++;
                    ...
                    foreach (var TransactionType in TransactionTypeList)
                    {
                        ...
                        foreach (var TTComponent in TTComponentList)
                        {
                            decimal Amount = Data.Where(x => x.OfficeShortOrder == OfficeDes && x.DepartmentName == Dept  && x.ComponentName == TTComponent.ComponentName).Sum(x => x.PRComponentAmount);
