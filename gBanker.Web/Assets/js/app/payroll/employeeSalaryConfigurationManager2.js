...
$(document).ready(function () {
    ...
    $("#SalaryScaleList").change(function () {
        ...
        if (scale != "") {
            ...
            if (grade != "" && empSalaryTypeId != "" && empSalaryTypeId != "") {
                if (system == 'GUP')
                    GenerateSalaryForEmployeeInPayScale_designation(empSalaryTypeId, grade, scale, employeeStatus, pfTypeId, EmployeeID);
    ...
    $("#btnSalaryConfigurationSave").click(function () {
        ...
        $.ajax({
            type: 'POST',
            contentType: "application/json; charset=utf-8",
            url: '/PRSalaryConfiguration/SalaryConfigurationSave2',
            data: JSON.stringify({
...
function LoadEmployeeInformationbyCode(employee_code) {
    // First AJAX Call: Fetch Employee Salary Configuration
    $.ajax({
        type: 'GET',
        contentType: "application/json; charset=utf-8",
        ...
        cache: false,
        success: function (data) {
            if (data.Result === "OK" && data.dataList !== null && data.dataList.length > 0) {
                GenerateTableWithData(data.dataList);

                let empData = data.dataList[0]; // Access first employee data

                // Salary Generation Type Handling
                if (empData.GradeId == 0) {
                ...
                hideUnhideDiv(); // Show/hide relevant sections
                ...
                $("#GradeList").val(empData.GradeId);

                if (empData.GradeId > 0) $("#GradeList").trigger("change");

                $("#EmployeeID").val(empData.EmployeeID);
                $("#EmployeeName").val(empData.EmployeeName);
                $("#EmployeeStatus").val(empData.EmployeeStatus);
                $("#txtEmpStatus").val(empData.EmployeeStatusName);
                $("#IsOvertimeException").prop('checked', data.IsOvertimeException);

                $("#EmployeeSalaryType").val(empData.EmployeeTypeId === 0 ? "" : empData.EmployeeTypeId);
                $("#SalaryScaleList").val(empData.Step === 0 ? "" : empData.Step);
                $("#FractionStep").val(empData.FractionStep === 0 ? "" : empData.FractionStep);
                ...
                let payrollConfigType = data.PayrollConfigurationType;
                let salaryValue = payrollConfigType === payrollConfigurationTypeEnum.GrossSalary
                    ? empData.GrossSalary : empData.BasicSalary;
                $("#GrossSalary").val(salaryValue);

                $("#EffectiveStartDate").val(empData.EffectiveStartDate);
                $("#EffectiveEndDate").val(empData.EffectiveEndDate);

                // Overtime Handling
                $("#IsOverTime").val("" + empData.IsOverTime);
                $("#MaxOvertimePerDay").val(empData.MaxOvertimePerDay);
                $("#MaxOvertimePerMonth").val(empData.MaxOvertimePerMonth);

                if (empData.IsOverTime) {
                    $(".section-overtime").show();
                } else {
                    $(".section-overtime").hide();
                }
                ...
                // Login Time Details
                $("#LoginTime").val(empData.LogInTime);
                $("#LogoutTime").val(empData.LogOutTime);
                $("#LastLoginTime").val(empData.LastLoginTime);
            } else {
                ...
                $('#tblSalaryConfiguration tbody').html("");
            }
        },
        error: function (request, status, error) {
    ...
    // Second AJAX Call: Fetch Routing Number
    $.ajax({
        type: 'GET',
        contentType: "application/json; charset=utf-8",
        url: '/PRSalaryConfiguration/GetRoutingNobyEmployeeCode',
        data: { employeeCode: employee_code },
        dataType: 'json',
        async: false,
        cache: false,
        success: function (data) {
                ...
                $("#RoutingNo").val(data.Result);
           
        },
        error: function (request, status, error) {
            console.log("Error fetching routing number: " + error);
        }
    });
}
