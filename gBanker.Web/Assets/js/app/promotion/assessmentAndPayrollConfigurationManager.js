...
function LoadEmployeeInformationbyCode(employee_code) {
    $.ajax({
        type: 'GET',
        contentType: "application/json; charset=utf-8",
        url: '/PromotionConfiguration/GetExistingSalaryConfigurationListbyEmployeeCodeNew',
        data: { employeeCode: employee_code, pid: $("#PromotionId").val() },
        dataType: 'json',
        async: false,
        cache: false,
        success: function (data) {
            if (data.Result === "OK") {
                ...
                if (data.dataList !== null) {
                    ...
                    $("#PreviousPromotionDate").val(data.PreviousPromotionDate);
                    $("#PreviousNextReviewDate").val(data.PreviousNextReviewDate);
