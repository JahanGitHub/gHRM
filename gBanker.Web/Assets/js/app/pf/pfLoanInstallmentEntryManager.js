...
function GetEmployeeByEmployeeCode(employeeCode) {
    var employeeId = '';
    $('#ContentPlaceHolder1_lblEmployee').text('');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var employeeInfo = `${data.EmployeeCode}, ${data.EmployeeName}`;

            $('#ContentPlaceHolder1_lblEmployee').text(employeeInfo);
...
function GetEmployeeByLoanType(loanTypeId, employeeId) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#InterestCharge').val(data.InterestCharge.toFixed(2));
...
function GetDisburseAndTodaysCollectionDetail(loanId) {
    var interestUptoOn = $('#InterestUptoOn').val();
    var employeeName = '';
    $.ajax({
        type: 'GET',
        contentType: "application/json; charset=utf-8",
        url: '/PFLoanCollection/GetDisburseAndTodaysCollectionUptoDetail',
        data: { loanId: loanId, interestUptoOn: interestUptoOn },
        dataType: 'json',
        async: false,
        success: function (result) {
            ...
            $('#InterestUpto').val(result.data.InterestUpto);
            $('#CurrentInterestCharge').val(result.data.CurrentInterestCharge);

            var totalDueAmountUpto = 'Total Due Amount Upto ' + interestUptoOn + ' :' + result.data.TotalDueAmountUpto;
            $('#TotalDueAmountUpto').html(totalDueAmountUpto);

            //populate to Print Loan Collection
            appLoanInsEntryManager.populatePrintLoanCollection(result.data);
        },
        error: function (request, status, error) {
        }
    });
...
var appLoanInsEntryManager = {
    init: function () {
        this.initDatePicker();
    },
    initDatePicker: function () {
        var dateToday = new Date();
        $(".interest-upto-datepicker").datepicker({
            dateFormat: "dd-M-yy",
            showAnim: "scale",
            changeMonth: true,
            changeYear: true,
            ...
            onClose: function (selectedDate) {
                $('#LoanTypeList').trigger('change');
            }
        });
        $(".interest-upto-datepicker").datepicker('setDate', new Date());
    },
    printLoanCollection: function (strid) {
    ...
    populatePrintLoanCollection: function (data) {

        $('#ContentPlaceHolder1_lblEmployee').text($("#EmployeeName").val());
        $('#ContentPlaceHolder1_lblLoan').text($("#LoanId").val());

        $('#ContentPlaceHolder1_lblDisburseDt').text($("#DisburseDate").val());

        $('#ContentPlaceHolder1_lblLoanAmt').text($("#DisburseAmount").val());
        $('#ContentPlaceHolder1_lblLoanPaid').text($('#LoanPaid').val());
        $('#ContentPlaceHolder1_lblLoanDue').text($('#PrincipalDue').val());

        var interestCharge = $('#InterestCharge').val();
        if (interestCharge) interestCharge = Number(interestCharge).toFixed(2);

        $('#ContentPlaceHolder1_lblInterestAmt').text(interestCharge);
        $('#ContentPlaceHolder1_lblInterestPaid').text($('#InterestPaid').val());
        $('#ContentPlaceHolder1_lblInterestDue').text($('#InterestDue').val());

        $('#ContentPlaceHolder1_lblTotDueAmt').text($("#TotalDue").val());
        $('#ContentPlaceHolder1_lblLastInstDt').text($("#TransactionDate").val());

        var interestUptoOn = $('#InterestUptoOn').val();

        $('#ContentPlaceHolder1_txtDateTo').val(interestUptoOn);

        $('#ContentPlaceHolder1_lblUptoInterest').text($("#CurrentInterestCharge").val());
       
        var totalDueAmountUpto = `Total Due Amount Upto ${interestUptoOn}`;

        $('#ContentPlaceHolder1_lblTotDueMsg').text(totalDueAmountUpto);
        $('#ContentPlaceHolder1_lblTotDueInt').text(data.TotalDueAmountUpto);

        $('#ContentPlaceHolder1_lblInstallment').text(data.TotalInstallmentNo);
        $('#ContentPlaceHolder1_lblInstComplete').text(data.TotalComplete);
        $('#ContentPlaceHolder1_lblInstDue').text(data.TotalNoDue);

        $('#ContentPlaceHolder1_lblStatus').text(data.CurrentStatus);
    },
};

$(function () {
    appLoanInsEntryManager.init();

    var trxDate = $('#TransactionDate').val();
    $('#InterestUptoOn').val(trxDate);
    ...
    $('#EmployeeCode').change(function () {
        ...
        selectedValue = "";
        generateDropdown("#LoanTypeList", url, selectedValue, "Please Select", "");
    });
