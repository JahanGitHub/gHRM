...
namespace gHRM.Web.Controllers.Loan
{
    public class LoanController : BaseController
    {
        ...
        [HttpGet]
        public JsonResult GetLoanNoByEmployeeForDropdown_Verc(int employeeId, bool? isClose)
        {
            var param = new
            {
            ...
            var alldata = employeeService.GetDataWithParameter(param, "GetLoanNoByEmployeeForDropdown");

            var lst = alldata.Tables[0].AsEnumerable()
                .Select(x => new SelectListItem
                {
                    Text = x.Field<string>("LoanNo"),
                    Value = x.Field<int>("LoanId").ToString()
                });
