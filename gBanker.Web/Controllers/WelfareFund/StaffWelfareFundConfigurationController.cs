#region Usings

using AutoMapper;
using gHRM.Core.Utilities.Constants;
using gHRM.Data.CodeFirstMigration.WelfareFund;
using gHRM.Service;
using gHRM.Service.StoreProcedure;
using gHRM.Service.WelfareFund;
using gHRM.Web.CommonDropdown;
using gHRM.Web.Helpers;
using gHRM.Web.Infrastructure.Date;
using gHRM.Web.ViewModels.WelfareFund.StaffWelfareFundConfiguration;
using gHRM.Web.ViewModels.WelfareFund.StaffWelfareFundSettings;
using Kendo.Mvc.Extensions;
using Kendo.Mvc.UI;
using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Web.Mvc;

#endregion

namespace gHRM.Web.Controllers.WelfareFund
{
    public class StaffWelfareFundConfigurationController : BaseController
    {
        #region Private Variables      
        private readonly IStaffWelfareFundSettingService staffWelfareFundSettingService;
        private readonly IStaffWelfareFundConfigurationService staffWelfareFundConfigurationService;
        private readonly IEmployeeSPService employeeService;
        public CommonStaticDropDown commonStaticDropDown;
        public CommonDynamicDropDown commonDynamicDropDown;

       

        #endregion

        #region Ctor
        public StaffWelfareFundConfigurationController(
            IStaffWelfareFundSettingService staffWelfareFundSettingService,
            IStaffWelfareFundConfigurationService staffWelfareFundConfigurationService,
            IEmployeeSPService employeeService
            )
        {
            this.staffWelfareFundSettingService = staffWelfareFundSettingService;
            this.staffWelfareFundConfigurationService = staffWelfareFundConfigurationService;
            this.employeeService = employeeService;
            commonStaticDropDown = new CommonStaticDropDown();
            commonDynamicDropDown = new CommonDynamicDropDown();
        }

        #endregion


        #region Listing

        public JsonResult GetStaffWelfareFundSettingListing([DataSourceRequest] DataSourceRequest request,
                  string searchTerm, string FilterColumn, string FilterValue)
        {
            try
            {
                List<StaffWelfareFundConfiguration> List_ViewModel = new List<StaffWelfareFundConfiguration>();


                var List = employeeService.GetDataWithoutParameter("SP_GET_Fund_List_Employee");


                List_ViewModel = List.Tables[0].AsEnumerable()
               .Select(row => new StaffWelfareFundConfiguration()
               {
                   StaffWelfareFundConfigurationId = row.Field<Int32>("StaffWelfareFundConfigurationId"),
                   EmployeeId = row.Field<int>("EmployeeId"),
                   PurposeId = row.Field<int>("PurposeId"),
                   FundAmount = row.Field<decimal>("FundAmount"),
                   remarks = row.Field<string>("remarks"),
              

               }).ToList();

                DataSourceResult result = List_ViewModel.ToDataSourceResult(request);
                return Json(new { data = result.Data, total = result.Total }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { Result = "ERROR", Message = ex.Message });
            }



        }



        public JsonResult GetPRpurposeListFund()
        {
            List<purposeListFund> List_ViewModel = new List<purposeListFund>();

            var empList = employeeService.GetDataWithoutParameter("sp_get_purposelistfund");
            List_ViewModel = empList.Tables[0].AsEnumerable()
               .Select(row => new purposeListFund
               {
                   Value = row.Field<int>("id"),
                   Text = row.Field<string>("purposename")
               }).ToList();

            return Json(List_ViewModel, JsonRequestBehavior.AllowGet);
        }


        public ActionResult Index()
        {
            var model = new StaffWelfareFundConfigurationViewModel();
            MapDropDownList(model);
            return View(model);
        }

        #endregion

        #region Ajax Calls      



        [HttpPost]
        public JsonResult Configure(StaffWelfareFundConfigurationViewModel model)
        {
            try
            {
                long createUser = Convert.ToInt64( SessionHelper.LoggedInEmployeeID);

       
                var resonse = staffWelfareFundConfigurationService.ConfigureStaffWelfareFund(model.EmployeeCode, model.PurposeId,  model.FundAmount, model.remarks, model.CreateUser);

                return Json(new { type = "success", message = "success" },
                           JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                //throw new Exception(ex.Message);
                return Json(new { type = "error", message = ex.Message },
                           JsonRequestBehavior.AllowGet);
            }
        }



        //[HttpPost]
        //public JsonResult Update(StaffWelfareFundConfigurationViewModel model)
        //{
        //    if (!ModelState.IsValid)
        //        return Json(new { type = "warning", message = "You must fill all the asteric(*) required fields." },
        //                    JsonRequestBehavior.AllowGet);


        //    var newStaffWelfareFundConfigurationViewModel =
        //                     Mapper.Map<StaffWelfareFundConfigurationViewModel, StaffWelfareFundConfiguration>(model);
        //    long createdBy = Convert.ToInt64(SessionHelper.LoggedInEmployeeID);

        //    var employees = employeeService.GetAll().Select(x => x).ToList();
        //    foreach (var employee in employees)
        //    {
        //        newStaffWelfareFundConfigurationViewModel.EmployeeId = Convert.ToInt32(employee.EmployeeId);
        //        newStaffWelfareFundConfigurationViewModel.StaffWelfareFundSettingId = model.StaffWelfareFundSettingId;
        //        newStaffWelfareFundConfigurationViewModel.CollectionDate = DateTime.Now;
        //        newStaffWelfareFundConfigurationViewModel.CreateUser = createdBy;
        //        newStaffWelfareFundConfigurationViewModel.IsActive = true;
        //        var response = staffWelfareFundConfigurationService.Create(newStaffWelfareFundConfigurationViewModel);
        //    }

        //    return Json(new { type = "success", message = "success" },
        //                      JsonRequestBehavior.AllowGet);
        //}

        #endregion

        #region Private Methods

        private void MapDropDownList(StaffWelfareFundConfigurationViewModel model)
        {
           
            var componentList = new List<SelectListItem>();
            componentList.Add(new SelectListItem() { Text = "Please Select", Value = "" });

            model.PurposeList = componentList;


        }



        #endregion
    }
}
