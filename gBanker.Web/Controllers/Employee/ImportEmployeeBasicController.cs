...
{
    public class ImportEmployeeBasicController : BaseController
    {
        ...
        #region Ctor
        public ImportEmployeeBasicController(
        ...
        public ActionResult ImportEmployeeBasic()
        {
            return View();
        }
                                                    ...
                                                    DataSet ds)
        {
            ...
            for (var i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(employeeCode))
                    newEmployee.EmployeeCode = CommonHelper.GetFormattedEmployeeCodeWithFourDigit(employeeCode);
                else
                    errorMessage += " Error: Employee Code not found in the file. " +
                                         "Row is " + (1 + i) + " and column is " + j;

                //national id
                var nationalID = ds.Tables[0].Rows[i][j++].ToString();

                if (!string.IsNullOrWhiteSpace(nationalID))
                    newEmployee.NationalId = nationalID;
                else
                    errorMessage += " Error: Employee NID not found in the file. " +
                                         "Row is " + (1 + i) + " and column is " + j;

                //address info
                string villageInfo = "", postInfo = "", thanaInfo = "", districtInfo = "", postCodeInfo = "";
                var village = ds.Tables[0].Rows[i][j++].ToString();

                var post = ds.Tables[0].Rows[i][j++].ToString();

                var thana = ds.Tables[0].Rows[i][j++].ToString();

                var district = ds.Tables[0].Rows[i][j++].ToString();

                var postCode = ds.Tables[0].Rows[i][j++].ToString();
                ...
                var permanentAddressInfo = $"{villageInfo}, {postInfo},{thanaInfo},{districtInfo} {postCodeInfo}";            
                    newEmployee.PermanentAddress = permanentAddressInfo;
                

                //emmergency contact no 
                var emmergencyContactInfo = ds.Tables[0].Rows[i][j++].ToString();
                if (!string.IsNullOrWhiteSpace(emmergencyContactInfo))
                {
                    newEmployee.ContactNo2 = emmergencyContactInfo;
                }


                ////spouse info
                //var spouseInfo = ds.Tables[0].Rows[i][j++].ToString();

                //if (!string.IsNullOrWhiteSpace(spouseInfo))
                //{
                //    newEmployee.Spouse = spouseInfo;
                //}

                ////height info
                //var heightInfo = ds.Tables[0].Rows[i][j++].ToString();

                //if (!string.IsNullOrWhiteSpace(heightInfo))
                //{
                //    newEmployee.Height = heightInfo;
                //}
                ...
                ////father name
                //var fatherName = ds.Tables[0].Rows[i][j++].ToString();

                //if (!string.IsNullOrWhiteSpace(fatherName))
                //{
                //    newEmployee. = fatherName;
                //}
