    ...
    public class RequisitionController : BaseController
    {
        ...
        public JsonResult GetAllRequsition(string reqType, bool isSelfStore, int jtStartIndex, int jtPageSize, string jtSorting = null)
        {
            ...
            try
            {
                ...
                List<Inv_RequsitionMasterViewModel> objList = new gHRMDBContext().Database.SqlQuery<Inv_RequsitionMasterViewModel>("sp_RequsitionHistory @reqType,@fromStoreID,@isSelfStore", arr).ToList();
        ...
        public JsonResult GetAllRequsitionDetails(int requsitionMasterID, string status)
        {
            ...
            if (status == "an")
            {
                ...
                {
                    var off = iOfficeService.GetById(LoginUserOfficeID.Value);
                    List<RequsitionAnalysisViewModel> objList = new gHRMDBContext().Database.SqlQuery<RequsitionAnalysisViewModel>("sp_Inv_RequsitionAnalysis " + requsitionMasterID + "," + off.OfficeLevel + "").ToList();
                    totalCount = objList.Count();
            ...
            else
            {
                ...
                try
                {
                    ...
                    List<RequsitionDetailViewmodel> objList = new gHRMDBContext().Database.SqlQuery<RequsitionDetailViewmodel>("sp_RequsitionDetails @reqMasterID,@dStatus", arr).ToList();
                    totalCount = objList.Count();
            ...
            if (LoginUserOfficeID.HasValue)
            {
                try
                {
                    ...
                    if (office.OfficeLevel != 4)
                    {
                        if (!storeID.HasValue)
                        {
                            List<InvWarehouse> wObj = new gHRMDBContext().Database.SqlQuery<InvWarehouse>("SELECT * FROM dbo.Inv_Warehouse WHERE OfficeID=" + LoginUserOfficeID + "").ToList();
                            storeID = wObj.First().WarehouseID;
                        }
                        List<Inv_RequisitionWiseTotalQtyInItemViewModel> objList = new gHRMDBContext().Database.SqlQuery<Inv_RequisitionWiseTotalQtyInItemViewModel>("Inv_sp_RequisitionWiseTotalQtyInItem " + storeID + "," + office.OfficeLevel + "").ToList();
                        totalCount = objList.Count();
        ...
        public JsonResult GetConsulateRequsitionAfterApproved()
        {
            ...
            if (LoginUserOfficeID.HasValue)
            {
                try
                {
                    ...
                    if (office.OfficeLevel != 4)
                    {
                        List<ConsulateRequsitionAfterApprovedViewModel> objList = new gHRMDBContext().Database.SqlQuery<ConsulateRequsitionAfterApprovedViewModel>("sp_Inv_ConsulateRequsitionAfterApproved " + LoginUserOfficeID.Value + "").ToList();
                        totalCount = objList.Count();
                    ...
                    if (dObj.Any())
                    {
                        using (var transaction = new TransactionScope())
                        {
                            try
                            {
                                ...
                                {
                                    int itemID = 0;
                                    if (new gHRMDBContext().inv_Settings.Where(x => x.Purpose == "Distribution"
                                ...
                                if (s > 0)
                                {
                                    ...
                                    if (s > 0)
                                    {
                                        ...
                                        if (storIn.Any())
                                        {
                                            ...
                                            var accList = new gHRMDBContext().Database.SqlQuery<AccChart>("SELECT * FROM AccChart WHERE AccCode in('2001','4535')").ToList();
                                            ...
                                            foreach (var acc in accList)
                                            {
                                                ...
                                                {
                                                    AccID = (int?)acc.AccChartID,
                                                    CreateDate = DateTime.Now,
                                                    CreateUser = LoginUserOfficeID.Value.ToString(),
                                                    Credit = (acc.AccountCode == "2001" ? 0 : totalPrice),
                                                    Debit = (acc.AccountCode == "2001" ? totalPrice : 0),
                                                    IsActive = true,
                                                    Narration = acc.AccountCode == "2001" ? "" : "Store In",
                                        ...
                                        else
                                        {
                                            ...
                                            foreach (var acc in accList)
                                            {
                                                Inv_TrxDetail d = new Inv_TrxDetail()
                                                {
                                                    ...
                                                    CreateDate = DateTime.Now,
                                                    CreateUser = LoginUserOfficeID.Value.ToString(),
                                                    Credit = (acc.AccountCode == "2001" ? totalPrice : 0),
                                                    Debit = (acc.AccountCode == "2001" ? 0 : totalPrice),
                                                    IsActive = true,
                                                    Narration = "",
                                                    TrxMasterID = a.TrxMasterID
                                                };
                ...
                try
                {
                    ...
                    foreach (var acc in accList)
                    {
                        Inv_TrxDetail d = new Inv_TrxDetail()
                        {
                            ...
                            Debit = (acc.AccountCode == "2001" ? 0 : totalPrice),
                            IsActive = true,
                            Narration = acc.AccountCode == "2001" ? naration : "",
                            TrxMasterID = a.TrxMasterID
                        };
                        iInvTrxDetailService.Create(d);
                    }
                            ...
                            else
                            {
                                ...
                                foreach (var acc in accList)
                                {
                                    Inv_TrxDetail d = new Inv_TrxDetail()
                                    {
                                        ...
                                        Debit = (acc.AccountCode == "2001" ? 0 : totalPrice),
                                        IsActive = true,
                                        Narration = acc.AccountCode == "2001" ? "Store Out" : "",
                                        TrxMasterID = a.TrxMasterID
                                    };
                                    iInvTrxDetailService.Create(d);
                                }
                        ...
                        using (gHRMDBContext db = new gHRMDBContext())
                        {
                            foreach (var o in obj)
                            {
                                Inv_RequsitionDispose d = new Inv_RequsitionDispose()
                                {
                                    ...
                                    RequestBy = SessionHelper.LoggedInEmployeeID,
        ...
        public JsonResult GetDisposeRequest(string status, string from, string to)
        {
            ...
            if (!string.IsNullOrEmpty(status) && !DateTime.MinValue.Equals(from) && !DateTime.MinValue.Equals(to))
            {
                try
                {
                    dispObj = new gHRMDBContext().Database.SqlQuery<RequsitionDisposeViewModel>("sp_DisposeRequest " +
        ...
        public JsonResult SendDisposeRequest(long DisposeRequestID, string status)
        {
            ...
            try
            {
                var warehouseObj = new gHRMDBContext().InvWarehouses.First(x => x.OfficeID == SessionHelper.LoginUserOfficeID);
                if (status == "r")
                {
                    using (gHRMDBContext db = new gHRMDBContext())
                    {
                        var obj = db.inv_RequsitionDisposes.First(x => x.DisposeRequestID == DisposeRequestID);
                        obj.RejectBy = SessionHelper.LoggedInEmployeeID;
                ...
                else if (status == "A" && SessionHelper.LoggedInOfficeDetail.OfficeLevel == 1)
                {
                    ...
                    {
                        var obj = db.inv_RequsitionDisposes.First(x => x.DisposeRequestID == DisposeRequestID);
                        obj.RequestApprovedBy = SessionHelper.LoggedInEmployeeID;
                ...
                else if (status == "D" && SessionHelper.LoggedInOfficeDetail.OfficeLevel == 2)
                {
                    using (gHRMDBContext db = new gHRMDBContext())
                    {
                        ...
                        if (storeObj.Any())
                        {
                            ...
                            {
                                obj.DisposeBy = SessionHelper.LoggedInEmployeeID;
                            ...
                            try
                            {
                                ...
                                foreach (var f in cObj)
                                {
                                    Inv_ConsolidateDisposeRequest c = new Inv_ConsolidateDisposeRequest()
                                    {
                                        ...
                                        ConsolidateBy = SessionHelper.LoggedInEmployeeID,
                                        ItemID = f.ItemID,
                                        Qty = f.Qty ?? 0,
                                        IsActive=true
                                    };
                                ...
                                foreach (var c in disObj)
                                {
                                    if (c.ApprovedQty == 0)
                                    {
                                        ...
                                        c.RejectBy = SessionHelper.LoggedInEmployeeID;
        ...
        {
            var dispObj = new gHRMDBContext().Database.SqlQuery<Inv_ConsolidateDisposeRequestViewModel>("inv_sp_ConsolidateDispose '" + from + "','" + to + "'").ToList();
            return Json(new { Result = "OK", Records = dispObj, TotalRecordCount = dispObj.Count() }, JsonRequestBehavior.AllowGet);
        }

        public JsonResult ConsolidateDisposeDataNew(string from, string to, int? officeID, int? itemID, string reportType)
        {
            var dispObj = new gHRMDBContext().Database.SqlQuery<Inv_ConsolidateDisposeRequestViewModel>("inv.inv_sp_ConsolidateDispose2 '" + from + "','" + to + "','HO','" + (reportType == null ? "" : reportType) + "'," + (officeID ?? 0) + "," + (itemID ?? 0) + "").ToList();
            return Json(new { Result = "OK", Records = dispObj, TotalRecordCount = dispObj.Count() }, JsonRequestBehavior.AllowGet);
        }
            ...
            using (gHRMDBContext db = new gHRMDBContext())
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    try
                    {
                        ...
                        cObj.ApprovedBy = SessionHelper.LoggedInEmployeeID;
                        ...
                        foreach (var o in disObj)
                        {
                            ...
                            o.RequestApprovedBy = SessionHelper.LoggedInEmployeeID;
            ...
            try
            {
                using (gHRMDBContext db = new gHRMDBContext())
                {
                    ...
                    var result = db.Database.SqlQuery<string>("inv_sp_ConsolidateDisposeApproved " + WarehouseID + ","+ warehouseObj.WarehouseID +
                        ","+Qty+",'"+ from + "','"+to+"',"+ SessionHelper.LoggedInEmployeeID + "").ToList();
                    msg = result.First();
                }
            }
            catch (Exception)
