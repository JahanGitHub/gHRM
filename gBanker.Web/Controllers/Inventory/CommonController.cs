...
namespace gHRM.Web.Controllers.Inventory
{
    public class CommonController : BaseController
    {
        ...
        {
            List<VendorViewModel> objList = new gHRMDBContext().Database.SqlQuery<VendorViewModel>(
                "SELECT v.*,w.WarehouseName FROM inv.Inv_Vendor v " +
                "INNER JOIN inv.Inv_Warehouse w ON v.WharehouseID=w.WarehouseID " +
                "WHERE v.IsActive=1 AND w.OfficeID={0}"
        ...
        public JsonResult GetAllEmployeeCodeXMaping(string empcode)
        {
            ...
            else
            {
                ...
                empCodeArr = iEmployeeService.GetMany(x => x.IsActive == true && x.EmployeeCode.Contains(empcode) && officeArr.Contains(x.OfficeId.Value)).Select(x => x.EmployeeCode);
            }
        ...
        {
            var zone = iIOfficeService.GetAllZoneOffice("", CompanyID ?? 0);
                ...
                else
                {
                    if (categoryID > 0)
                    {
                        if (string.IsNullOrEmpty(itemCode) | string.IsNullOrWhiteSpace(itemCode) | categoryID == 0)
                            ...
                            obj = new gHRMDBContext().Database.SqlQuery<InvStoreExistPriceViewModel>("exec inv_sp_storePriceXItem " + officeID + ",'" + itemCode + "','" + itemName + "',"+categoryID+"").ToList();
        ...
        public JsonResult NoGenerate(string prefix)
        {
            if (LoginUserOfficeID.HasValue)
            {
                ...
                {
                    var req = db.Database.SqlQuery<string>("exec inv.sp_Inv_NoGenerate {0},'" + prefix + "'", (LoginUserOfficeID ?? 0)).ToList();
