...
namespace gBanker.Web.Controllers.Inventory
{
    public class StoreController : BaseController
    {
        ...
        private readonly ICompanyService iOrganizationService;
        ...
        ICompanyService iOrganizationService,
        ...
        public ActionResult StoreList()
        {
            if (LoginUserOfficeID.HasValue)
            {
                ...
                if (SessionHelper.LoggedInRoleId == 12 | SessionHelper.LoggedInRoleId == 6
                    | SessionHelper.LoggedInRoleId == 1) allowMultipleoffice = 1;
        ...
        {
            List<InvWarehouse> listObj = new gHRMDBContext().Database.SqlQuery<InvWarehouse>("exec sp_InvOfficeMapping {0}", (LoginUserOfficeID ?? 0)).ToList();
        ...
        public JsonResult GetStoreDetailXGrid(int jtStartIndex, int jtPageSize, string jtSorting, string filterValue)
        {
            try
            {
                ...
                {
                    objList = db.Database.SqlQuery<Inv_StoreViewModel>("exec inv.sp_StoreDetails " + (LoginUserOfficeID ?? 0) + "," + 54 + "").ToList();
                }
        ...
        public JsonResult GetWarehouseItems()
        {
            ...
            using (gHRMDBContext db = new gHRMDBContext())
            {
                objList = db.Database.SqlQuery<Inv_StoreItemViewModel>("exec inv.sp_StoreDetailsItem " + (LoginUserOfficeID ?? 0) + "").ToList();
                if (objList.Any())
        ...
        public JsonResult GetWarehouseItemsXCategory(int categoryID)
        {
            ...
            using (gHRMDBContext db = new gHRMDBContext())
            {
                objList = db.Database.SqlQuery<Inv_StoreItemViewModel>("exec inv.sp_StoreDetailsItem2 " + (LoginUserOfficeID ?? 0) + "," + categoryID + "").ToList();
                if (objList.Any())
                    objList = objList.Where(x => x.Qty > 0).ToList();
            }
            return Json(new { Records = objList }, JsonRequestBehavior.AllowGet);
                ...
                if (obj.Any())
                {
                    using (TransactionScope transactionScope = new TransactionScope())
                    {
                        try
                        {
                            ...
                            var accList = new gHRMDBContext().Database.SqlQuery<AccChart>("SELECT * FROM AccChart WHERE AccountCode in(" + srt + ")").ToList();
                            ...
                            foreach (var acc in accList)
                            {
                                Inv_TrxDetail d = new Inv_TrxDetail()
                                {
                                    ...
                                    Credit = ((obj.First().StockType == "O" && acc.AccountCode == "2001") ? totalPrice :
                                    (obj.First().StockType == "I" && acc.AccountCode == "4535") ? totalPrice : 0),
                                    Debit = ((obj.First().StockType == "O" && acc.AccountCode == "6101") ? totalPrice :
                                    (obj.First().StockType == "I" && acc.AccountCode == "2001") ? totalPrice : 0),
                                    IsActive = true,
                                    Narration = acc.AccountCode == "2001" ? "" : "Store " + (obj.First().StockType == "O" ? "Out" : "In"),
        ...
        public JsonResult StoreTypeXDetails(string storeType, string date, string slno, string officeCode)
        {
                ...
                !string.IsNullOrEmpty(date) | !string.IsNullOrEmpty(slno)) && !string.IsNullOrEmpty(officeCode))
            {
                ...
                //var widlst = new gHRMDBContext().Database.SqlQuery<int>("select w.WarehouseID from dbo.Inv_Warehouse w " +
        ...
        public JsonResult StoreItemBalanceUpdateXID(long stockID, int qty, string status, decimal UnitPrice)
        {
            ...
            try
            {
                var st = iInvStoreService.GetById((int)stockID);
                #region Update
                if (status == "u")
                {
                    if (qty > 0)
                ...
                else if (status == "d")
                {
                    if (st.StockType == "O" || st.StockType == "D")
                    {
                        ...
                        {
                            var refSt = iInvStoreService.GetById((int)st.RefStoreID.Value);
                            if (refSt == null)
                                refSt = new Inv_Store();
                            if (refSt.ID > 0)
                            {
        ...
        public JsonResult GetItemwiseStore(int itemID)
        {
            ...
            try
            {
                var objList = new gHRMDBContext().Database.SqlQuery<ItemwiseStoreViewModel>("sp_inv_ItemwiseStore {0},{1}", itemID, LoginUserOfficeID.Value).ToList();
                cnt = objList.Count();
                ...
                try
                {
                    if (v != null)
                    {
                        ...
                        if (sqty.Any())
                        {
                            ...
                            var accList = new gHRMDBContext().Database.SqlQuery<AccChart>("SELECT * FROM AccChart WHERE AccountCode in('2001','6101')").ToList();
                            Inv_TrxMaster m = new Inv_TrxMaster()
                            {
                                CreateDate = DateTime.Now,
                                CreateUser = LoginUserOfficeID.ToString(),
                            ...
                            foreach (var acc in accList)
                            {
                                Inv_TrxDetail d = new Inv_TrxDetail()
                                {
                                    ...
                                    IsActive = true,
                                    Narration = acc.AccountCode == "2001" ? "" : "Item Dispose",
                                    TrxMasterID = a.TrxMasterID
                                };
                                iInvTrxDetailService.Create(d);
                            }
